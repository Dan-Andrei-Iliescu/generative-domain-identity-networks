...
train_path = os.path.join('data', 'mnist', 'mnist_train.txt')
test_path = os.path.join('data', 'mnist', 'mnist_test.txt')
save_path = os.path.join('results', 'mnist_final')
    ...
    for row in range(samples.shape[0]):
        for col in range(samples.shape[1]):
            grid[SHAPE*row:SHAPE*(row+1), SHAPE*col:SHAPE*(col+1)] = \
                samples[row, col].reshape(SHAPE, SHAPE)
